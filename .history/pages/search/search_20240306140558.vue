<template>
	<view>
		<!-- 微信小程序端导航栏 -->
		<!-- #ifdef MP -->
		<view class="flex align-center fixd-top bg-white" style="height: 44px;">
			<!-- 搜索框 -->
			<input type="text" placeholder="点击搜索作品/视频"
				class="iconfont icon-sousuoxiao ml-2 flex align-center flex-1 rounded-circle py-1 px-1"
				style="background-color: #f6f7f8; color: #959aa0;" v-model="keyword">
			<!-- 按钮选项 -->
			<view class="flex align-center px2 mx-2" style="color: #959aa0;" @click="handleSearch">搜索</view>
		</view>
		<!-- #endif -->
		<media-list v-for="(item, index) in list" :key="index" :item="item" :index="index">
		</media-list>
		<!-- 默认提示 -->
