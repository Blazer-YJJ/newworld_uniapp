<template>
	<view style="line-height: 1; overflow-y: hidden;">
		<!-- #ifndef APP-PLUS -->
		<video :src="src" :poster="poster" controls style="height: 225px; width: 750rpx;" id='myVideo'>
		</video>
		<!-- #endif -->
		<scroll-view scroll-y="true" :style="'height:' + scrollHeight + 'px;'">
			...
			<view v-if="tabIndex === 0" class="animated fadeIn fast">
				<view class="px-3 py-2 flex align-center">
					<image :src="detail.user.avatar || '/static/demo/cate/1%20(2).png'" mode="aspectFill"
						style="width: 80rpx; height: 80rpx;" class="ml-3 rounded-circle mr-3" @click="openUserSpace">
					</image>
					...
					<view class="flex align-center justify-center rounded bg-main text-white px-2 py-1 ml-auto"
						hover-class="bg-main-hover" v-if="detail.user.id !== user_id" @click="isFollow">
				...
				<view class="px-3 flex justify-between align-center text-muted">
					...
					<view class="flex align-center justify-center rounded bg-main text-white px-2 py-1 ml-auto"
						hover-class="bg-main-hover" v-if="detail.user.id !== user_id" @click="isFava">
				...
				<scroll-view scroll-x="true" class="scroll-row py-2">
					<view class="scroll-row-item border text-muted mx-1 p-1"
						style="height: 120rpx; width: 250rpx;border-radius: 25rpx;" v-for="(item, index) in videos"
						:key="index"
				...
				<view class="f-list">
					<media-list v-for="(item, index) in hotVideoList" :key="index" :item="item" :index="index">
					</media-list>
				</view>
			</view>
			<!-- 评论 -->
			<view v-else class="animated fadeIn fast px-3 py-1">
				<view class="uni-comment-list" v-for="(item, index) in comments" :key="index">
					<view class="uni-comment-face">
						<image :src="item.send_user.avatar || '/static/demo/cate/1%20(11).png'"
							style="width: 80rpx; height: 80rpx;" class="rounded-circle" mode="aspectFill">
						</image>
					</view>
					<!-- 一级评论 -->
					<view class="uni-comment-body">
						...
						<!-- 二级评论 -->
						<view class="felx flex-wrap rounded p-2 bg-light" v-for="(item2, index2) in item.comments"
							:key="index2">
							<text class="font text-main">{{ item2.send_user.username || item2.send_user.nickname
								}}</text>
							<text class="font text-dark">回复：</text>
							<text class="font text-main">{{ item2.reply_user.username || item2.reply_user.nickname
								}}:</text>
				...
				<view class="position-fixed flex align-center justify-center bg-main rounded-circle"
					...
					hover-class="animated fast pulse bg-main-hover" @click="openComment(0)">
		...
		<!-- 选集弹出框 -->
		<f-popup ref="popup">
			<view class="position-absolute bottom-0 right-0 left-0 bg-white animated faster fadeIn" @click.stop="stop">
				<view style="height: 650rpx;">
					<view class="flex align-center px-3 py-2">
						<text class="font-md">合集</text>
						<view style="height: 80rpx; height: 80rpx;" class="ml-auto font-md" hover-class="text-main"
							@click="closeTab">
					...
					<scroll-view scroll-y="true" style="height: 540rpx;">
						<view class="flex flex-wrap">
							<view style="width: 50%; box-sizing: border-box;" v-for="(item, index) in videos"
								:key='index' class="p-2">
								<view class="border text-muted pl-1" style="height: 120rpx; border-radius: 25rpx;"
									:class="activeIndex === index ? 'border-main text-main' : 'text-muted border-light-secondary'"
									@click="changeVideo(item, index)">
									<text class="font-md">第{{ index + 1 }}集</text>
									<view class="font-sm text-ellipsis">
		...
		<f-popup ref="comment">
			<view class="position-absolute bottom-0 left-0 right-0 bg-white animated faster fadeIn" @click.stop="stop">
				<view style="height: 100rpx;" class="flex align-center px-3">
					<input type="text" class="bg-light rounded mr-3 px-2 flex-1" style="height: 60rpx;"
						:placeholder="reply_user.id ? '回复@' + reply_user.name : '皮一下~'" v-model="content" />
					<view class="flex align-center justify-center rounded bg-main text-white py-1 px-2 ml-auto"
						hover-class="bg-main-hover" @click="sendContent">
