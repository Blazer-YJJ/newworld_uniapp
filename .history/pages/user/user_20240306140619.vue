<template>
  <view>
    <view>
      <navigator v-if="user" :url="'../userSpace/userSpace?user_id=' + user.id">
        <view class="p-3 flex align-center">
          <image
            :src="user.avatar || '/static/demo/cate/titleimg.png'"
            class="user-img bg-light rounded-circle flex-shrink"
          ></image>
          <view class="flex flex-column pl-3 flex-1">
            <view class="font-md text-dark"
              >{{ user.username || user.nickname
              }}<text class="font-sm text-light-muted ml-3">{{ user.sex }}</text></view
            >
            <text class="font-sm text-muted">{{
              user.desc || "这个人很懒~暂无描述"
            }}</text>
          </view>
        </view>
      </navigator>
      <navigator v-else url="../login/login">
        <view class="p-3 flex align-center">
          <image
            src="/static/demo/cate/titleimg.png"
            class="user-img bg-light rounded-circle flex-shrink"
          ></image>
          <view class="flex flex-column pl-3 flex-1">
    ...
    <view class="py-2 px-3">
      <main-big-button @click="addVideo"
        ><text class="iconfont icon-shipintianchong px-2"></text>立即投稿
      </main-big-button>
    </view>
    <!-- 按钮弹框 -->
    <f-popup ref="popup">
      <view
        class="position-absolute bottom-0 right-0 left-0 bg-white animated faster fadeIn"
      >
        ...
        <view class="f-divider"></view>
        <view
          class="py-2 flex align-center justify-center font-md"
          hover-class="bg-light"
...
export default {
  ...
  onShow() {
    this.$H
      ...
      .then((res) => {
        (this.followCount = res.followCount), (this.videoCount = res.videoCount);
      });
  },
  // 监听原生button
